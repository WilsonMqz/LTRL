...
class Im_CIFAR10(VisionDataset):
    ...
    def __init__(self, root, imb_type='exp', imb_factor=0.01, seed=0, train=True, extend_size=5, fix_size=50,
                 transform=None, target_transform=None, download=False):

        super(Im_CIFAR10, self).__init__(root, transform=transform,
        ...
        self.data, self.targets = self.gen_imbalanced_data(img_num_list)

        self.extend_size = extend_size
        self.fix_szie = fix_size
    ...
    def generate_TL_SL(self):
        n = len(self.targets)  # sample size
        K = max(self.targets) + 1  # class size
        c = K - self.fix_szie  # extend size of stochastic label
        new_TL = copy.deepcopy(self.targets)
        new_SL = torch.zeros(n, K)
        fix_SL = list(range(c, K))
        avgC = 0

        S = self.extend_size

        print("extend size:{}".format(self.extend_size))
        for i in range(n):

            # fixed num
            SL = random.sample(range(c), S)
            SL.extend(fix_SL)
            SL = np.sum(np.eye(K)[SL], axis=0)

            SL = torch.from_numpy(SL)
            while torch.sum(SL) == 1:
                r_m = random.sample(range(K), S)
                for j in range(len(r_m)):
                    SL[r_m[j]] = 1
            avgC += torch.sum(SL)
            if SL[self.targets[i]] == 1:
                new_SL[i] = torch.from_numpy(np.ones(K))
                new_SL[i][self.targets[i]] = 0
            else:
                new_TL[i] = K
                new_SL[i] = SL
        ...
        new_SL_num = len(new_TL_array[np.where(new_TL_array == K)])
